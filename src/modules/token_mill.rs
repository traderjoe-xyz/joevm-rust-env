use alloy::{dyn_abi::DynSolValue, primitives::aliases::U96, sol, sol_types::SolCall};
...
use crate::{commons::DEFAULT_ADDRESS, engine::EvmEngine};
...
sol!(TMFactory, "src/bindings/TMFactory.json");
sol!(TMStaking, "src/bindings/TMStaking.json");
sol!(TMERC20, "src/bindings/TMERC20.json");
sol!(TMMarket, "src/bindings/TMMarket.json");
...
impl TokenMillManager for EvmEngine {
    fn deploy_token_mill(
        &mut self,
        default_protocol_fee_share: u16,
        default_referral_fee_share: u16,
    ...
    fn create_token_and_market(
        ...
        let base_token_decimals = DynSolValue::Uint(U256::from(token_decimals), 8).abi_encode();

        let parameters = ITMFactory::MarketCreationParameters {
            tokenType: U96::from(1),
...
#[cfg(test)]
mod tests {
    use crate::modules::tokens::TokenCreator;
