...
sol!(ERC20, "src/bindings/ERC20.json");
...
impl TokenCreator for EvmEngine {
    fn create_token(&mut self, name: &str, decimals: u8) -> Result<Address> {
    ...
    fn balance_of(&mut self, token: &str, owner: Address) -> Result<u128> {
        ...
        let data: Vec<u8> = ERC20::balanceOfCall { account: owner }.abi_encode();
        ...
        match result {
            ExecutionResult::Success {
                ...
                ERC20::balanceOfCall::abi_decode_returns(&data, false)?._0,
            )?),
...
mod tests {
    ...
    const ALICE: Address = address!("0000000000000000000000000000000000000002");
    const BOB: Address = address!("0000000000000000000000000000000000000003");
